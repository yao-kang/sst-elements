...
Description : This sub-python script parses the snapshot xml file that is dumped by the scheduler. It then creates a runscript for ember simulation and runs the script. Created specifically for detailed simulation of network congestion together with scheduling & application task mapping algorithms. Updated January 2017: All intermediate files and output files can be redirected to an output folder. This was multiple experiments can be launched at the same time without intefering with each other. Experimental settings such as link bandwidth ratio, link arrangement, or routing algorithm can be fed as parameters to the script.
'''
...
def parse_xml (options):
    ...
    for job in jobs:
        ...
        #Fulya Jan 2016: Commented out sorting. We had it as a practical way of task mapping in ember, we do not need it any more.
