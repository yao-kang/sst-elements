...
namespace SST {
    namespace Vanadis {
        class VanadisREMInstruction : public VanadisMathInstruction {

        public:
            VanadisREMInstruction(SST::Output* output, const uint32_t id, const uint64_t iAddr,
			const uint32_t rRd, const uint32_t rR1, const uint32_t rR2) :
                ...
                std::string toString() {
                    std::string instStr = "REM " + generateRegisterString();
                    return instStr;
                }
