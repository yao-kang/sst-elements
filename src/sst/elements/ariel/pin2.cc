...
ArielPin2Interface::ArielPin2Interface(ComponentId_t id, Params& params, uint32_t core_count, ArielMemoryManager* memmgr) :
            ArielAppInterface(id, params, core_count, memmgr) {
...
bool ArielPin2Interface::launch()
{
...
int ArielPin2Interface::forkPINChild(const char* app, char** args, std::map<std::string, std::string>& app_env) {
...
#ifdef HAVE_CUDA
GpuReturnTunnel* ArielPin2Interface::getGpuReturnTunnel() {
    return tunnelR;
}

GpuDataTunnel* ArielPin2Interface::getGpuDataTunnel() {
    return tunnelD;
}
#endif

void ArielPin2Interface::tick() {
    tunnel->updateTime(getCurrentSimTimeNano());
    tunnel->incrementCycles();
}

bool ArielPin2Interface::getCommandNB(uint32_t coreID, ArielCommand* ac) {
    return tunnel->readMessageNB(coreID, ac);
}

ArielCommand ArielPin2Interface::getCommand(uint32_t coreID) {
    return tunnel->readMessage(coreID);
}

ArielPin2Interface::~ArielPin2Interface() {
    delete tunnel;
#ifdef HAVE_CUDA
    delete tunnelR;
    delete tunnelD;
...
void ArielPin2Interface::emergencyShutdown() {
