...
/** End memory manager subcomponent parameter translation */
    ...
    app = loadUserSubComponent<ArielAppInterface>("app", ComponentInfo::SHARE_NONE, core_count, memmgr);
    if (!app) {
        app = loadAnonymousSubComponent<ArielAppInterface>("ariel.interface.pin2", "app", 0, ComponentInfo::INSERT_STATS, params, core_count, memmgr);
    }
    ...
    for(uint32_t i = 0; i < core_count; ++i) {
        cpu_cores.push_back(loadComponentExtension<ArielCore>(app, 
...
void ArielCPU::init(unsigned int phase)
{
    if ( phase == 0 ) {
        if (!app->launch()) {
            output->fatal(CALL_INFO, -1, 0, "Error, %s: Application failed to launch\n", getName().c_str());
        }
    }
