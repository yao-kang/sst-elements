...
class TopoJSON(Topo):
    def __init__(self, topo_path, rtr_path):
        Topo.__init__(self)
        self.topo_path = topo_path
        self.rtr_path = rtr_path
        self.node_numbering = {}
    ...
    def buildComponents(self):
      for i in xrange(self.num_routers):
        ...
        rtr = sst.Component("rtr.%d" % i, "merlin.hr_router")
        rtr.addParam("id", i)
        rtr.addParam("num_ports", str(num_ports))
        rtr.addParam("topology", "merlin.json")
        for p in "link_bw", "flit_size", "xbar_bw", "input_latency", "output_latency", "input_buf_size", "output_buf_size", "xbar_arb":
          if p in _params:
            rtr.addParam(p, _params[p])
        ...
        for port in range(num_ports):
          ...
          if dest.is_rtr:
            ...
            ep = self._getEndPoint(dest.id).build(dest.id, {})
            if ep:
              nicLink = sst.Link("nic.%d:%d" % (i,port))
              if self.bundleEndpoints:
                nicLink.setNoCut()
              nicLink.connect(ep, (rtr, "port%d" % port, _params["link_lat"]))
