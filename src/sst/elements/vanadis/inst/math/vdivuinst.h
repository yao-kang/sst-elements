...
namespace SST {
    namespace Vanadis {
        class VanadisDIVUInstruction : public VanadisMathInstruction {

        public:
            VanadisDIVUInstruction(SST::Output* output, const uint32_t id, const uint64_t iAddr,
			const uint32_t rRd, const uint32_t rR1, const uint32_t rR2) :
                ...
                std::string toString() {
                    std::string instStr = "DIV " + generateRegisterString();
                    return instStr;
                }
