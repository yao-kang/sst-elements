 ...
 * Author: sdhammo
 *
 * Created on January 13, 2017, 4:15 PM
 */
...
namespace SST {
    namespace Vanadis {
        class VanadisORInstruction : public VanadisMathInstruction {
            
        public:
            VanadisORInstruction(SST::Output* output, const uint32_t id, const uint64_t iAddr,
			const uint32_t rRd, const uint32_t rR1, const uint32_t rR2) :
                ...
                std::string toString() {
                    std::string instStr = "OR " + generateRegisterString();
                    return instStr;
                }
